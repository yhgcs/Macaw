...
class ShapeRenderer: NodeRenderer {
	...
	func render() {
		let hash = (shape.form as! Path).segments.count
		var cachedImage = ShapeRenderer.cachedImages[hash]
		if cachedImage == .None {
			UIGraphicsBeginImageContext(CGSizeMake(100.0, 100.0))
			let cgContext = UIGraphicsGetCurrentContext()
			setGeometry(shape.form, ctx: cgContext!)
			drawPath(shape.fill, stroke: shape.stroke, ctx: cgContext!)
			cachedImage = UIGraphicsGetImageFromCurrentImageContext()
			ShapeRenderer.cachedImages[hash] = cachedImage
			UIGraphicsEndImageContext()
		}

		let cgContext = ctx.cgContext
		CGContextDrawImage(cgContext, CGRectMake(0.0, 0.0, 100.0, 100.0), cachedImage!.CGImage)
	}
